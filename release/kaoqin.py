...
for i in range (0,data1['人员编号'].unique().size):
    ...
    afternoon = 0
    evening = 0
    last_sunday_evening = mdata[i*7][(mdata[i*7] != 0)*(mdata[i*7] <= 240)] #上周日晚上的记录
    ...
    for j in range (1,8):
        ...
        flag2 = 0
        if j != 7:
            ax = mdata[num][(mdata[num+1] != 0)*(mdata[num+1] <= 240)] #打卡超过零点的记录
            a = ax.shape[0]
        bx = mdata[num][(mdata[num] != 0)*(mdata[num] > 240)*(mdata[num] <= 680)] #上午打卡的记录
        b = bx.shape[0]
        cx = mdata[num][(mdata[num] != 0)*(mdata[num] > 850)*(mdata[num] <= 1040)] #下午打卡的记录
        c = cx.shape[0]
        dx = mdata[num][(mdata[num] != 0)*(mdata[num] > 1040)*(mdata[num] <= 1439)] #晚间打卡的记录
        d = dx.shape[0]
        ex = mdata[num][(mdata[num] != 0)*(mdata[num] >= 680)*(mdata[num] <= 850)] #中午打卡的记录
        e = ex.shape[0]
        if b != 0:
            if e == 0:
                morning = morning +  np.max(bx) - np.min(bx) #早退
            elif e >= 1:
        ...
        if c == 0:
            ...
            if e > 1 and d == 0 and flag1 == 0:
                afternoon = afternoon + np.max(cx) - np.max(ex)
            elif e > 1 and d == 0 and flag1 == 1:
                afternoon = afternoon + np.max(cx) - np.max(ex)
            elif e > 1 and d != 0 and flag1 == 0:
